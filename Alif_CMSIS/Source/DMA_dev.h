...
typedef enum {
    ...
    DMA_CHANNEL_CRC_MODE            = (1 << 2),         /*!< CRC: Skip peripheral flush and wait */
...
typedef struct _DMA_THREAD_INFO {
    ...
    uint8_t               dma_mcode[DMA_MICROCODE_SIZE];/*!< DMA microcode built for this transaction */
...
typedef struct
{
    ...
    struct {
        ...
        __IM dma_ccr_t CCR;                    /*!< 0x8 Channel Control of Channeln */
...
typedef struct _DMA_DRV_INFO {
    ...
    DMA_SECURE_STATE         apb_intf;                /*!< DMA interface to be used       */
