...
void camera_demo(void)
{
    ...
    printf("\n \t frame buffer pool size: 0x%08" PRIX32 "  pool addr: 0x%" PRIxPTR " \r\n ",
           (uint32_t)FRAME_BUFFER_POOL_SIZE, (uintptr_t)framebuffer_pool);
    ...
    printf("Ulink: ~/%s_%dx%d.bin 0x%" PRIXPTR " 0x%" PRIXPTR "\n",
           SELECTED_CAMERA_SENSOR, FRAME_WIDTH, FRAME_HEIGHT,
           (uintptr_t)framebuffer_pool,
           (uintptr_t)(framebuffer_pool + FRAME_BUFFER_POOL_SIZE - 1));

    printf("T32:\n   data.save.binary ~/cam_image0_%s_%dx%d.bin 0x%" PRIxPTR
           "--0x%" PRIXPTR " \r\n",
           SELECTED_CAMERA_SENSOR, FRAME_WIDTH, FRAME_HEIGHT,
           (uintptr_t)framebuffer_pool,
           (uintptr_t)(framebuffer_pool + FRAME_BUFFER_POOL_SIZE - 1));
