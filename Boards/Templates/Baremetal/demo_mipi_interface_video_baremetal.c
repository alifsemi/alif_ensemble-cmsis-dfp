 ...
 */
/**************************************************************************//**
 * @file     demo_mipi_interface_video_baremetal.c
 * @author   Suhail Ahmed H I
 * @email    suhail.ahmed@alifsemi.com
 ******************************************************************************/
/* System Includes */
#include <stdio.h>
#include <inttypes.h>
...
/*Number of bytes per pixel in RGB565*/
...
void Display_callback(uint32_t event)
{
    ...
    if (event & ARM_CDC_DSI_ERROR_EVENT) {
        /* Transfer Error: Received Hardware error, Wake-up Thread. */
    	pending_events |= DISP_CB_EVENT_ERROR;
    }
}
...
/* Allocated memory address for
    ...
    /* Convert the camera captured image into RGB565,
    ...
    for (;;) {
    	...
    	ret = CAMERAdrv->CaptureFrame((void *)framebuffers);
    	if (ret != ARM_DRIVER_OK) {
...
#else
    	/*Start Camera and capture image in given frame buffer*/
        ret = CAMERAdrv->CaptureFrame(cam_framebuffer_pool);
        if (ret != ARM_DRIVER_OK) {
        ...
        __disable_irq();
        actual_events = pending_events;
        pending_events = 0U;
        __enable_irq();

    	if ((actual_events & CAM_CB_EVENT_ERROR) ||
    	        (actual_events & DISP_CB_EVENT_ERROR)) {
				/* Error: Camera or Display failed. */
			printf("\r\n \t\t >> Error: Camera or Display failed. \r\n");
			goto error_poweroff_camera;
		}

		if (actual_events & CAM_CB_EVENT_CAPTURE_STOPPED){

			/* we have received Success: Camera Capture Frame stop detected. */

	#if BOARD_CAMERA_HAS_STREAM_BIT_ENABLED
			uint8_t *src_frame_buffer =
				(uint8_t *)framebuffers[completed_buf_idx & (N_FRAMEBUFF - 1U)];

			ret = aipl_demosaic_rgb565(src_frame_buffer,
			...
			ret = aipl_demosaic_rgb565(cam_framebuffer_pool,
									   bayer_to_rgb_buffer_pool,
									   CAM_FRAME_WIDTH,
									   CAM_FRAME_WIDTH,
									   CAM_FRAME_HEIGHT,
